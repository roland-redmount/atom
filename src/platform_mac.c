/**
 * This is the platform layer for Mac OS
 * 
 * TODO: this is largerly copied from platform_linux.c, with some modifications.
 * We should factor out the identical implementations to a common file platform_maxos_linux.c
 */
...
/**
 * Create a file. If size > 0 the file is resized to the given size.
 */
static bool createFile(char const * fileName, int * fileDescriptor, size64 size)
{
	*fileDescriptor = open(
		fileName,
		O_RDWR | O_CREAT,
...
{
	int fileDescriptor;
	if(!createFile(fileName, &fileDescriptor, size)) {
		fileMapping->size = 0;
